...
foreach ( $weekdays as $i => $weekday ) {
	...
	if ( !$skip_day ) {
		...
		$tabs .= '<div class="master-schedule-tabs-headings">' . $newline;
		$tabs .= '<div class="master-schedule-tabs-day-name"';
		if ( !$atts['display_date'] ) {
			$tabs .= ' title="' . esc_attr( $date_subheading ) . '"';
		}
		$tabs .= '>' . esc_html( $display_day ) . '</div>' . $newline;
		if ( $atts['display_date'] ) {
			$tabs .= '<div class="master-schedule-tabs-date">' . esc_html( $date_subheading ) . '</div>' . $newline;
		}
		$tabs .= '</div>' . $newline;

		$tabs .= '<div class="shift-right-arrow">' . $newline;
		$tabs .= '<a href="javacript:void(0);" onclick="return radio_shift_tab(\'right\');" title="' . esc_attr( __( 'Next Day', 'radio-station' ) ) . '">' . $arrows['right'] . '</a>' . $newline;
		$tabs .= '</div>' . $newline;
		$tabs .= '<div id="master-schedule-tab-bottom-' . strtolower( $weekday ) . '" class="master-schedule-tab-bottom"></div>' . $newline;
		// 2.3.2: add start and end day times for automatic highlighting
		$tabs .= '<span class="rs-time rs-start-time" data="' . esc_attr( $day_start_time ) . '"></span>' . $newline;
		$tabs .= '<span class="rs-time rs-end-time" data="' . esc_attr( $day_end_time ) . '"></span>' . $newline;
		$tabs .= '</li>';
		...
		if ( count( $shifts ) > 0 ) {
			...
			foreach ( $shifts as $shift ) {
				...
				if ( $atts['show_image'] ) {
					...
					if ( $show_avatar ) {
						$avatar = '<div class="' . esc_attr( $classlist ) . '">' . $newline;
						if ( $show_link ) {
						...
						$avatar = '<div class="' . esc_attr( $classlist ) . '"></div>' . $newline;
					}
					$avatar = apply_filters( 'radio_station_schedule_show_avatar_display', $avatar, $show['id'], 'tabs' );
					// $panels .= $avatar;
				}
				...
				$title .= '</div>' . $newline;
				$title = apply_filters( 'radio_station_schedule_show_title_display', $title, $show['id'], 'tabs' );
				...
				if ( $atts['show_hosts'] ) {
					...
					$show_hosts = apply_filters( 'radio_station_schedule_show_hosts', $show_hosts, $show['id'], 'tabs' );
					if ( $show_hosts ) {
						$hosts = '<div class="show-dj-names show-host-names">' . $newline;
						$hosts .= $show_hosts;
						$hosts .= '</div>' . $newline;
						$hosts = apply_filters( 'radio_station_schedule_show_hosts_display', $hosts, $show['id'], 'tabs' );
				...
				$show_time = apply_filters( 'radio_station_schedule_show_time', $show_time, $show['id'], 'tabs', $shift );
				...
				$display = apply_filters( 'radio_station_schedule_show_times_display', true, $show['id'], 'tabs', $shift );
				if ( !$display ) {
				...
				if ( $atts['show_encore'] ) {
					...
					if ( 'on' == $show_encore ) {
						...
						$encore .= '</div>' . $newline;
						$encore = apply_filters( 'radio_station_schedule_show_encore_display', $encore, $show['id'], 'tabs' );
				...
				if ( $atts['show_genres'] ) {
					...
					$genres .= '</div>' . $newline;
					$genres = apply_filters( 'radio_station_schedule_show_genres', $genres, $show['id'], 'tabs' );
				...
				$classlist = implode( ' ', $classes );
				$panels .= '<div class="' . esc_attr( $classlist ) . '">' . $newline;
				...
				if ( $atts['show_desc'] ) {
					...
					$show_excerpt = apply_filters( 'radio_station_schedule_show_excerpt', $show_excerpt, $show['id'], 'tabs' );

					// --- output excerpt ---
					$excerpt = '<div class="show-desc">' . $newline;
					$excerpt .= $show_excerpt . $newline;
					$excerpt .= '</div>' . $newline;
					$excerpt = apply_filters( 'radio_station_schedule_show_excerpt_display', $excerpt, $show['id'], 'tabs' );
