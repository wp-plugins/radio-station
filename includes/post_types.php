...
function radio_station_top_meta_boxes() {
    global $post; do_meta_boxes( get_current_screen(), 'top', $post );
}
...
function radio_station_myplaylist_inner_custom_box() {
    ...
    echo "<tr>";
    echo "<th></th><th><b>".__('Artist', 'radio-station')."</b></th>";
    echo "<th><b>".__('Song', 'radio-station')."</b></th>";
    echo "<th><b>".__('Album', 'radio-station')."</b></th>";
    echo "<th><b>".__('Record Label', 'radio-station')."</th>";
...
function radio_station_myplaylist_inner_metainfo_custom_box() {
    ...
    <input type="checkbox" name="show_active" <?php if ( $active == 'on' ) {echo 'checked="checked"';} ?>  />
...
function radio_station_myplaylist_inner_sched_custom_box() {
	...
	?>
	        ...
	        foreach ( $shifts[0] as $track ){
	            if ( isset( $track['day'] ) || isset( $track['time'] ) ){
	            	...
	            	<ul style="list-style:none;">
						...
						<li style="display:inline-block; margin-left:20px;"><input type="checkbox" name="show_sched[<?php echo $c; ?>][encore]" <?php if ( isset( $track['encore'] ) && ( $track['encore'] == 'on' ) ) {echo 'checked="checked"';} ?>/> <?php _e( 'Encore Presentation', 'radio-station' ); ?></li>

						<li style="display:inline-block; margin-left:20px;"><span class="remove button-secondary" style="cursor: pointer;"><?php _e( 'Remove', 'radio-station' ); ?></span></li>
	    ...
	    ?>
	<span id="here"></span>
	<center><a class="add button-primary" style="cursor: pointer; display:block; width: 150px; padding: 8px; text-align: center; line-height: 1em;"><?php echo __('Add Shift', 'radio-station'); ?></a></center>
	<script>
...
function radio_station_myplaylist_save_showpostdata( $post_id ) {
	...
	if ( !is_array( $djs ) ) {$djs = array();}
	else {
		foreach ( $djs as $i => $dj ) {
			if ( $dj != '' ) {
				$userid = get_user_by( 'id', $dj );
				if ( !$userid ) {unset( $djs[$i] );}
			}
		}
	}
	$file = strip_tags( trim( $_POST['show_file'] ) );
	...
	foreach ( $scheds as $i => $sched ) {
		foreach ( $sched as $key => $value ) {
			...
			if ( $key == 'day' ) {
				...
				if ( $value == '' ) {$isvalid = true;}
				elseif ( ( absint($value) > -1 ) && ( absint($value) < 61 ) ) {$isvalid = true;}
			...
			if ( $isvalid ) {$new_scheds[$i][$key] = $value;} else {$new_scheds[$i][$key] = '';}
		}
	}
	...
	<div id="meta_inner" class="sched-override">
		...
		<ul style="list-style:none;">
			<li style="display:inline-block;">
				...
				<input type="text" id="OverrideDate" name="show_sched[date]" value="<?php if ( isset( $override['date'] ) && ( $override['date'] != '' ) ) {echo $override['date'];} ?>"/>
			</li>
			...
			<li style="display:inline-block; margin-left:20px;">
				...
				<select name="show_sched[start_hour]" style="min-width:35px;">
					<option value=""></option>
				<?php for ( $i = 1; $i <= 12; $i++ ) {
					if ( isset( $override['start_hour'] ) && ( $override['start_hour'] == $i ) ) {$selected = ' selected="selected"';} else {$selected = '';}
					echo '<option value="'.$i.'"'.$selected.'>'.$i.'</option>';
				} ?>
				</select>
				...
				<?php for ( $i = 0; $i < 60; $i++ ) {
					$min = $i;
					if ( $i < 10 ) {$min = '0'.$i;}
					if ( isset( $override['start_min'] ) && ( $override['start_min'] == $min ) ) {$selected = ' selected="selected"';} else {$selected = '';}
					echo '<option value="'.$min.'"'.$selected.'>'.$min.'</option>';
				} ?>
				</select>
				<select name="show_sched[start_meridian]" style="min-width:35px;">
					<option value=""></option>
					<option value="am"<?php if ( isset( $override['start_meridian'] ) && ( $override['start_meridian'] == 'am' ) ) {echo ' selected="selected"';} ?>>am</option>
					<option value="pm"<?php if ( isset( $override['start_meridian'] ) && ( $override['start_meridian'] == 'pm' ) ) {echo ' selected="selected"';} ?>>pm</option>
				</select>
			</li>
			...
			<li style="display:inline-block; margin-left:20px;">
				...
				<select name="show_sched[end_hour]" style="min-width:35px;">
					<option value=""></option>
				<?php for ( $i = 1; $i <= 12; $i++) {
					if ( isset( $override['end_hour'] ) && ( $override['end_hour'] == $i ) ) {$selected = ' selected="selected"';} else {$selected = '';}
					echo '<option value="'.$i.'"'.$selected.'>'.$i.'</option>';
				} ?>
				</select>
				...
				<?php for ( $i = 0; $i < 60; $i++ ) {
					$min = $i;
					if ( $i < 10 ) {$min = '0'.$i;}
					if ( isset( $override['end_min'] ) && ( $override['end_min'] == $min ) ) {$selected = ' selected="selected"';} else {$selected = '';}
					echo '<option value="'.$min.'"'.$selected.'>'.$min.'</option>';
				} ?>
				</select>
				<select name="show_sched[end_meridian]" style="min-width:35px;">
					<option value=""></option>
					<option value="am"<?php if ( isset( $override['end_meridian'] ) && ( $override['end_meridian'] == 'am' ) ) {echo ' selected="selected"'; } ?>>am</option>
					<option value="pm"<?php if ( isset( $override['end_meridian'] ) && ( $override['end_meridian'] == 'pm' ) ) {echo ' selected="selected"'; } ?>>pm</option>
				</select>
			</li>
		</ul>
	</div>
