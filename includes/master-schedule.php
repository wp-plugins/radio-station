...
function radio_station_master_schedule( $atts ) {
	...
	$show_shifts = $wpdb->get_results(
		...
		JOIN {$wpdb->prefix}postmeta AS active 
	...
	foreach ( $show_shifts as $shift ) {
		...
		foreach ( $shift->meta_value as $time ) {
			...
			if ( 'pm' === $time['start_meridian'] && 12 !== $time['start_hour'] ) {
				$time['start_hour'] += 12;
			}
			if ( 'am' === $time['start_meridian'] && 12 === $time['start_hour'] ) {
				$time['start_hour'] = 0;
			}

			if ( 'pm' === $time['end_meridian'] && 12 !== $time['end_hour'] ) {
				$time['end_hour'] += 12;
			}
			if ( 'am' === $time['end_meridian'] && 12 === $time['end_hour'] ) {
				$time['end_hour'] = 0;
			}
		...
		foreach ( $master_list as $hour => $days ) {
			...
			foreach ( $days as $day => $min ) {
				...
				foreach ( $min as $shift ) {
					...
					if ( 0 === $shift['time']['start_hour'] && 0 === $shift['time']['end_hour'] ) { ///midnight to midnight shows
						if ( $shift['time']['start_min'] === $shift['time']['end_min'] ) { //accomodate shows that end midway through the 12am hour
							$rowspan = 24;
						}
					}

					if ( 0 === $shift['time']['end_hour'] && 0 !== $shift['time']['start_hour'] ) { //fix shows that end at midnight (BUT take into account shows that start at midnight and end before the hour is up e.g. 12:00 - 12:30am), otherwise you could end up with a negative row span
